...
### CSS 는 섹션별로 미디어쿼리가 붙어 있다

형제 폴더의 "토큰 → 레이아웃 → 컴포넌트 → 반응형" 순서와 달리, 여기서는 각 컴포넌트 블록
바로 뒤에 해당 `@media` 가 따라온다. 새 반응형 규칙도 파일 끝이 아니라 **해당 컴포넌트 옆에** 쓴다.
...
1. **Pretendard 가 실제로 로드되지 않는다.** `index.html` 의 `<link>` 가
   `fonts.googleapis.com/css2?family=Pretendard…` 를 가리키는데 Google Fonts 에는 Pretendard 가
   없어 **HTTP 400** 이 떨어진다(확인함). 지금 화면은 `-apple-system` 폴백으로 렌더되는 중이다.
   같은 저장소의 `0004`/`0005` 가 쓰는 방식으로 바꾸면 해결된다:
