...
`clamp()` 의 최소/최대 모두 이 범위 안에 있어야 한다. **예외는 두 개뿐이다**: `.hero-wordmark`
(`clamp(72px,15vw,132px)`, 2026-09-03 사용자 요청으로 확정)와 `.form-agree`/`.form-agree-more`
(동의 체크박스 라벨·"전문보기" 링크, `16px`, 2026-09-11 사용자 요청으로 확정 — 본문과 같은
눈높이로 읽힐 필요 없는 보조 요소라는 이유) — 다른 요소에 이 예외들을 유추 확장하지 않는다.
