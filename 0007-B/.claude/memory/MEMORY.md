...
- [헤드리스 시각 검증법](gopumgyeok-headless-verification.md) — 브라우저 확장 없음; 포트 재사용 전 kill 필수; `--window-size`<450px는 내부 500px로 왜곡됨(≥800px 사용); `--virtual-time-budget`은 transform 애니메이션을 왜곡할 수 있어 CDP 실시간 교차검증 병행
- [Swiper creative+loop 중앙정렬 회귀](gopumgyeok-swiper-creative-loop-bug.md) — effect:'creative'+loop:true 조합에서 활성 슬라이드가 밀려 보임(실기기 재현) → fade 유지 + 별도 CSS 키프레임으로 우회
...
- [폼 범위 셀렉터 특이성 함정](feedback_form-scoped-selector-specificity-trap.md) — #inquiryForm/.inquiry-sheet-form 안 일반 input/label 규칙이 새 컴포넌트의 좁은 클래스를 3번이나 조용히 덮어씀(가로 스크롤 버그까지 유발)
